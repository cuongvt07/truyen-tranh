...
<div class="container">
    <header class="header-manga" style="margin-bottom:14px">
        <div class="container" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
            ...
            <a href="{{ route_path('collections.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> {{ __('messages.community.create_collection') }}</a>
        </div>
    </header>
        ...
        @forelse($items as $c)
            <div class="my-story">
                ...
                <div class="my-story__actions">
                    <a href="{{ route_path('collections.edit', $c->id) }}" class="btn btn-invincible"><i class="fa fa-edit"></i></a>
                    <form method="post" action="{{ route_path('collections.destroy', $c->id) }}" onsubmit="return confirm('{{ __('messages.community.delete_collection_confirm') }}')" style="display:inline">@csrf @method('delete')<button class="btn btn-invincible" style="color:#e84040"><i class="fa fa-trash"></i></button></form>
                </div>
            </div>
        @empty
            <div class="nothing" style="padding:40px 0;text-align:center;color:var(--meta-color)">
                ...
                {{ __('messages.community.no_collections') }} <a href="{{ route_path('collections.create') }}">{{ __('messages.community.create_now') }}</a>
            </div>
        @endforelse
