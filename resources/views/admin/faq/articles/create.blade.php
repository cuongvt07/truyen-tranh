...
@section('template_title', 'Create FAQ Article')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Create FAQ Article</h3>
    </div>
    <form action="{{ route('admin.faq.articles.store') }}" method="POST">
        @csrf
        <div class="card-body">
            <div class="form-group">
                ...
                <select class="form-control @error('category_id') is-invalid @enderror" id="category_id" name="category_id" required>
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" {{ old('category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->title_en }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('content_en') is-invalid @enderror" id="content_en" name="content_en" rows="10" required>{{ old('content_en') }}</textarea>
                @error('content_en')<span class="invalid-feedback">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="content_vi">Content (VI)</label>
                <textarea class="form-control @error('content_vi') is-invalid @enderror" id="content_vi" name="content_vi" rows="10">{{ old('content_vi') }}</textarea>
            ...
            <div class="form-check">
                <input type="checkbox" class="form-check-input" id="comments_enabled" name="comments_enabled" value="1" {{ old('comments_enabled', true) ? 'checked' : '' }}>
